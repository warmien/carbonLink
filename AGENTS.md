...
- **开发历程文件**：项目根目录 `DEVLOG.md`，记录所有版本开发历程
- **记录触发时机**：
  1. 每完成一个版本（V1/V2/V3...）的所有编码任务后，**必须更新 DEVLOG.md**
  2. 每次推送到 GitHub 后，**必须更新 DEVLOG.md** 的提交记录表
  3. 每次重大功能变更（新增模块/架构调整/核心逻辑重构），**必须更新 DEVLOG.md**
- **记录内容要求**：
  - 版本号 + 完成日期
  - 新增/修改/删除的文件清单及变更说明
  - 核心逻辑变更的简要描述
...
- **格式规范**：
  ...
  - 文件变更使用表格展示
  - 待办事项使用 `- [ ]` 复选框
- **自动化规则**：在执行 `git commit` 之前，先检查 DEVLOG.md 是否需要更新，如需要则先更新再提交